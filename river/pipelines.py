...
class RiverPipeline:
    ...
    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `river`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM river WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO river (date, title, link, content)"
